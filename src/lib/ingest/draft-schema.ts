...
3. **Emit operations** (\`ops\`) — usually 1–25. Each op is one Dexie row write the document implies. Possible kinds:
   ...
   - \`add_lab_result\` — for lab reports. Populate the field per analyte the document reports. Always include \`date\` (ISO YYYY-MM-DD, the collection/sampling date — not the report print date) and \`source\` ("epworth" or "external"). **One op per distinct collection date.** Multi-page reports, historical trend tables, and longitudinal "compare-to-previous" printouts routinely contain several sampling dates on a single document — emit one \`add_lab_result\` op per date, with only the analytes that were actually measured on that date. Never collapse different collection dates into one op. If the same analyte is shown for multiple dates (trend column), split it out accordingly.
