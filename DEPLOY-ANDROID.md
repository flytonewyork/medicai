...
### A. GitHub Actions (preferred for ongoing releases)

Once the keystore + secrets are wired (see "One-time CI secrets"
below), every release is a single button-press:

1. Actions tab → **APK build** workflow → *Run workflow*.
2. Fill in `version` (semver, e.g. `1.0.1`), optional `notes_en` /
   `notes_zh`.
3. The workflow runs bubblewrap, signs with the keystore, calls
   `scripts/apk-publish.mjs`, and opens a PR titled
   `chore(apk): publish <version>` containing:
   - `public/apk/anchor-<version>.apk` — the signed binary
   - `public/apk/manifest.json` — updated to set this version as `latest`
4. Review the PR (size + sha256 sanity check), merge to `main`,
   Vercel redeploys, the new APK is live at:
   - `https://PROD_HOST/apk` — versioned install page (the share link)
   - `https://PROD_HOST/apk/latest.apk` — stable redirect to the
     latest signed APK
   - `https://PROD_HOST/apk/anchor-<version>.apk` — pinned download

### B. Local build (one-off)
...
`pnpm apk:publish` copies the signed APK into `public/apk/`, computes
its SHA-256 + size, and writes the new release to
`public/apk/manifest.json` with the latest pointer set. Commit both
the new `.apk` and the manifest update on a branch, open a PR, merge.

## One-time CI secrets

For the GitHub Actions path, set these repo secrets
(Settings → Secrets and variables → Actions → New repository secret):

| Secret | Value |
| --- | --- |
| `ANDROID_KEYSTORE_BASE64` | `base64 -i android/android.keystore` (one line, no wrapping). Generated by `bubblewrap init`. |
| `ANDROID_KEYSTORE_PASSWORD` | Keystore password chosen during `bubblewrap init`. |
| `ANDROID_KEY_PASSWORD` | Key password (same as keystore unless you set them separately). |
| `TWA_PROD_HOST` | Production host without protocol, e.g. `anchor.thomashu.com`. Substituted into `twa-manifest.json` at build time. |

The keystore secret is the only piece that is genuinely irreplaceable.
Lose it and you can never publish an update under the same package id —
back it up to a password manager **outside** this repo.
...
## Iteration — code fixes ship without a new APK

The APK is intentionally a thin TWA shell. It contains:

- the package id + icon + name (set in `twa-manifest.json`)
- the production URL it points at (`https://PROD_HOST/`)
- the SHA-256 fingerprint Chrome uses to verify the assetlinks endpoint

**No app code is bundled inside the APK.** Every launch loads the
live `https://PROD_HOST/` page, which is whatever Vercel's most
recent production deploy serves. Push to `main`, Vercel deploys, and
the next time dad opens Anchor (or pulls to refresh inside it) he sees
the new version. There is nothing to reinstall.

The service worker (`public/sw.js`) deliberately avoids fetch-caching
— it only handles push notifications — so a fresh load is a fresh load.
The build commit SHA is shown at the bottom of `/apk` so dad (or you)
can confirm the running build matches `main`.

The only time you rebuild and redistribute the APK is when:
- the icon, colors, package id, or app name change (edit
  `twa-manifest.json`, bump `appVersion`, run `bubblewrap update`
  then `bubblewrap build` — or just bump `version` and let the
  `APK build` workflow handle it; the workflow auto-increments
  `appVersion`)
- the signing key rotates (rare; update `TWA_SHA256_FINGERPRINTS` in
  Vercel **before** sending the new APK, and rotate
  `ANDROID_KEYSTORE_BASE64` in repo secrets)
